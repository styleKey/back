프록시 객체 초기화
1. 조회를 했을 때
2. 영속성 컨텍스트에서 초기화를 요청
3. 영속성 컨텍스트는 db에서 조회해서 실제 엔티티를 생성한다.
4. 그러면 프록시 객체에 있는 타겟이 연결되어서 조회해 올 수 있다.

프록시 특징
- 프롲ㄱ시 객체는 처음 사용할 때 한번만 초기화
- 프록시 객체를 초기화 한다고 프록시 객체가 실제엔티티로 바뀌는 것이 아니라 실제 엔티티에 접근이 가능하다.
- 프록시 객체는 원본 엔티티를 상속받기 때문에 instance of를 사용해야함


조회 시 영속성 컨텍스트에 이미 엔티티가 잇으면 실제 엔티티를 반환한다.
