...
public class CoordinateLookApiController {
    ...
    @GetMapping
    @Operation(summary = "Read All CoordinateLook", description = "코디룩 정보 전체 조회 (좋아요 많은 순으로 정렬), 무한 스크롤")
    public ApiResponse<ApiCoordinateLookPageResponse> getCoordinateLooks(
            @PageableDefault(sort = "likeCount", direction = Direction.DESC) Pageable pageable) {
        return ApiResponse.ok(coordinateLookService.getAllCoordinateLooksWithLikes(pageable));
    }
